...
void data_cache_t::read(inst_t *m_inst) {
    ...
    if(block) { // Cache hit
        ...
        block_t victim_block = victim_cache->remove(addr); // check victim cache first, before checking memory
        if (victim_block.valid) { // if victim cache hit
            victim_block.tag = tag; // convert from victim to data cache tag format
            block_t evicted = blocks[set_index][0]; // block that will be evicted from data cache
            blocks[set_index][0] = victim_block; // swapping victim cache with data cache
            evicted.tag = (evicted.tag << (set_offset - block_offset)) | set_index; // reconstruct full address tag
            victim_cache->insert(evicted); // Insert evicted block into victim cache
            read(m_inst);  // read operation again, which will hit
            return; //return to avoid memory request
        }
        missed_inst = m_inst;
...
void data_cache_t::write(inst_t *m_inst) {
    ...
    else { // Cache miss, same as read operation.
        block_t victim_block = victim_cache->remove(addr); // check victim cache before memory access
        if (victim_block.valid) { // if victim cache hit
            victim_block.tag = tag; // convert from victim to data cache tag format
            block_t evicted = blocks[set_index][0]; // block that will be evicted from data cache
            blocks[set_index][0] = victim_block; // swapping victim cache with data cache
            evicted.tag = (evicted.tag << (set_offset - block_offset)) | set_index; // reconstruct full address tag
            victim_cache->insert(evicted); // Insert evicted block into victim cache
            write(m_inst); // write operation again, which will hit
            return; //return to avoid memory request
        }
        missed_inst = m_inst;
        memory->load_block(addr & ~block_mask, block_size);
...
void data_cache_t::handle_response(int64_t *m_data) {
    ...
    if(victim->valid) {
        victim->tag = (victim->tag << (set_offset - block_offset)) | set_index; // Convert data cache tag to victim cache tag format
        victim_cache->insert(*victim); // Insert evicted block into victim cache
#ifdef DEBUG
...
block_t victim_cache_t::remove(uint64_t m_addr) {
    ...
    num_accesses++; // increment for every victim cache access
    
    uint64_t tag = m_addr >> 5; // calculate tag by removing block offset
    for (uint64_t i = 0; i < num_entries; i++) { // search through entries available, starting from most recent.
        if (blocks[i].valid && blocks[i].tag == tag) { // match found
            num_hits++; // increment victim cache hit
            block = blocks[i]; // block
            for (uint64_t j = i + 1; j < num_entries; j++) { // shift blocks for fifo
                blocks[j - 1] = blocks[j];
            }
...
void victim_cache_t::insert(block_t m_block) {
    ...
    if(num_entries == size) {
        ...
        for (uint64_t i = 1; i < size; i++) {  // fifo algorithm:
            blocks[i - 1] = blocks[i];
        }
        num_entries--; // Decrement count
    }
